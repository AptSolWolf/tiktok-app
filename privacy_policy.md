...
This Privacy Policy describes how this application ("App") handles information in connection with its operation. This App is a personal content management tool used to publish video content to a TikTok account via the TikTok Content Posting API. It is operated by a single developer for personal use only.
...
Since this App is for personal use only, the following data is **NOT** collected:
...
All data is stored **locally on the developer's own machine** or within private automation tools (e.g., n8n self-hosted instance). No data is transmitted to external servers beyond what is required by the TikTok API itself.

***

## 4. TikTok API Data Usage

This App uses the TikTok Content Posting API to publish videos to a TikTok account. By using the TikTok API, the App is subject to TikTok's own privacy practices. TikTok may collect data as described in their Privacy Policy available at:
...
- `video.publish` — to post videos to the authorized TikTok account
...
- **Custom desktop application** — developed privately, not distributed to third parties
...
All API credentials and tokens are stored securely on the developer's local machine or private environment. No credentials are shared, published, or transmitted beyond authenticated API calls to TikTok's official endpoints.
